    ...
    def run_bot(self):
        ...
        def get_query(message):
            ...
            if message.from_user.id == self.videos_adder:
                ...
                if (
                    len(message_splitted) == 2
                    and message_splitted[0] == "add_new_video_by_link"
                ):
                    added = self.video_parser.add_video(message_splitted[1])
                    if added:
                        msg = self.bot.send_message(
                            message.chat.id, f"Video added successfully"
                        )
                        self.talker.boot()
                    else:
                        msg = self.bot.send_message(
                            message.chat.id, f"Video wasn't added, check link"
                        )
